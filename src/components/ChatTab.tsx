...
export default function ChatTab() {
  const { messages, addMessage, clearMessages, isProcessing, setIsProcessing, uploadedFile } = useApp();
  ...
  const handleSend = async () => {
    ...
    try {
      ...
      addMessage("chat", createMessage("assistant", "I apologize, but I encountered an error. Please try again."));
    } finally {
        ...
        {chatMessages.length === 0 ? (
          <div className="flex items-center justify-center h-full">
            <div className="text-center">
              ...
              <p className="text-white/20 text-xs mt-1">Your intelligent assistant for Rwanda and the world</p>
            </div>
          </div>
        ) : (
